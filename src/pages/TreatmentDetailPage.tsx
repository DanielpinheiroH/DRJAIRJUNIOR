...
export default function TreatmentDetailPage() {
  ...
  return (
    <>
      ...
      <section className="treatment-hero"><div className="container"><Breadcrumb items={breadcrumbs} /><div className="treatment-hero-grid"><div><span className="eyebrow">{treatment.category}</span><h1>{treatment.title}</h1><p>{treatment.shortDescription}</p><WhatsAppButton origin="treatment_hero" message={treatmentWhatsAppMessage(treatment.title)} /></div><MediaPlaceholder alt={`Imagem relacionada a ${treatment.title}`} expectedFile={treatment.imageFile} /></div></div></section>
